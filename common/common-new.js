/**
* @file Módulo com funções comuns de uso geral
...
//--- Definição de compatibilidade para o uso em Extensões Web ---

var browser = (browser ?? null) || (chrome ?? null);


//--- Prototipagens (definições em common.d.ts) ---
...
if (!String.prototype.toDate) {
	...
	String.prototype.toDate = function () {
		...
		if (this.match(/\s*(?:hoje|today|now)\s*/i)) return new Date();
		if (this.match(/\s*(?:ontem|yesterday)\s*/i)) return new Date().addDate("-1d");
		if (this.match(/\s*(?:amanh[aã]|tomorrow)\s*/i)) return new Date().addDate("+1d");
...
if (!Object.prototype.valueOfKey) {
	Object.defineProperty(Object.prototype, 'valueOfKey', {
		value: function (key, value) {
			...
			while (target && (key = keys.shift())) {
				if (target[key] === undefined) {
					if (value === undefined) return undefined;
					target[key] = keys.length ? {} : value;
				} else if (!keys.length && arguments.length > 1) {
...
/**
 * Verificar se elementos estão dentro do Viewport
 * @function external:"jQuery.fn".inViewport
 * @returns {Boolean}
 */
$.fn.inViewport = function () {
	...
	var viewportTop = parent.offsetTop + parent.scrollTop
	var viewportBottom = viewportTop + $(parent).height();
	var viewportLeft = parent.offsetLeft + parent.scrollLeft
	var viewportRight = viewportLeft + $(parent).width();


	var result = elementTop >= viewportTop && elementBottom <= viewportBottom &&
...
/**
 * Adiciona aos elementos uma barra de ações
 * @function external:"jQuery.fn".actionable
 * @param {ActionableOptions} options 
